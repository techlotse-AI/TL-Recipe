...
- Video detection now also finds the player on pages that use neither `<article>` nor `<main>`, and on pages that keep it outside those landmarks, so imports from sites with simpler markup no longer come back without a video. Lookalike hosts (e.g. `evilyoutube.com`) are rejected rather than treated as YouTube.
